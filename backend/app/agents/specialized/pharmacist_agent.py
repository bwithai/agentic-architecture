...
class PharmacistAgent:
    ...
    async def _execute_product_search(self, search_filters: Dict[str, Any], limit: int) -> List[Dict[str, Any]]:
        ...
        # If no products found with specific filters, return empty list
        if not all_products:
            print(f"\n🔄 No products found matching patient's specific symptoms and conditions")
            print(f"   This case will be handled by providing personalized assistance")
    ...
    async def generate_product_recommendations(self, patient_data: json, max_recommendations: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            if not products:
                print("No matching products found - will return empty recommendations for specialized handling")
                # Return structure with empty recommendations instead of error
                patient_info = patient_data.get("patient", {})
                return {
                    "patient_info": {
                    ...
                    "consultation_report": "No matching products found in current database. This case requires specialized consultation.",
